...
class ChessController {
    ...
    public void onSquareClick(int row, int col) {
        if (selectedSquare == null) {
            ...
            Piece piece = model.getBoardState()[selectedSquare.x][selectedSquare.y];
            Set<Point> validMoves = piece.getValidMoves(model.getBoardState(), selectedSquare.x, selectedSquare.y);
            
            if (validMoves.contains(targetSquare)) {
                model.getBoardState()[targetSquare.x][targetSquare.y] = piece;
                model.getBoardState()[selectedSquare.x][selectedSquare.y] = null;
                view.updateBoard(model.getBoardState());
            } else {
